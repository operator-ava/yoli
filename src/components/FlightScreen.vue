...
// Экран «Перелёт». Сверху — ДАТА ПРИЛЁТА в Узбекистан, ниже прежний текст
// «скоро»: рейсов нет, сроков не обещаем. Дата живёт здесь, потому что
// человек сначала решает, когда прилетает, и только потом — сколько живёт.
//
// Тап по числу в календаре закрывает и календарь, и этот экран: возврат
// к расчёту одним действием. Крестик и кнопка внизу закрывают без выбора —
// дата тогда не меняется.
//
...
function apply(iso: string) {
  ...
  // Дата выбрана — экран сделал своё дело и закрывается сам. Человек
  // возвращается к расчёту одним тапом, а не тремя. Расчёт под экраном
  // не трогается: он всё это время оставался на месте.
  emit('close')
}
